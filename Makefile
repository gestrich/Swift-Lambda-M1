...
builder-bot:
  ...
  # Remove the -l from back command as thought it was causing issues with swift package but not sure if any relation?? 'l' invokes a login shell
	docker run  --platform linux/amd64 --rm -v $($@BUILD_DIR):/build-target -v `pwd`:/build-src -w /build-src builder bash -c "swift package resolve -v; swift build --product $($@PRODUCT) -c release --build-path /build-target"

	# copy deps
  # For some reason the awk statement fails if you run this Makefile directly without aws sam :/
	-docker run --platform linux/amd64 --rm -v $($@BUILD_DIR):/build-target -v `pwd`:/build-src -w /build-src builder bash -cl "ldd '/build-target/release/$($@PRODUCT)' | grep swift | awk '{print $3}' | xargs cp -Lv -t /build-target/lambda"
